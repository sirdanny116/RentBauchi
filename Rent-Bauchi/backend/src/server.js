...
const WHATSAPP_LINK = process.env.WHATSAPP_LINK || "https://wa.me/2348012345678";
...
function propertyView(data,p){
  ...
  return {
    ...
    platformShare: Math.round(annualRent * 0.05),
...
// Serve the public renter/owner website and the separate Admin portal from one
// Node process. The portals remain separate URLs, while API authorization
// enforces the role boundary on the server.
...
app.post("/api/auth/forgot", authLimiter, async (req,res)=>{
  ...
  return res.json({
    message: demoMode
      ? "Reset link generated. Use the token below (demo mode)."
...
app.post("/api/agent/nin",auth,roles("agent"),upload.single("nin"),(req,res)=>{
...
app.post("/api/agent/properties/:id/media",auth,roles("agent"),upload.array("media",10),(req,res)=>{
...
app.get("/api/admin/analytics",auth,roles("admin"),(req,res)=>{
  ...
  const platformShare=(p)=>Math.round(Number(p.annualRent||0)*0.05);
...
async function prepareDemoAccounts(){
  const data=db();
  const demos={
    "renter@rentbauchi.test":"123456",
    "agent@rentbauchi.test":"123456",
    "admin@rentbauchi.test":"Admin123"
  };
  let changed=false;
  for(const u of data.users){
    if(demos[u.email]){
      u.passwordHash=await bcrypt.hash(demos[u.email],10);
      changed=true;
    }
  }
  if(changed) save();
...
if (require.main === module) {
  prepareDemoAccounts().then(()=>{
    app.listen(PORT,()=>console.log(`Rent Bauchi API running on http://localhost:${PORT}`));
  });
}

function createApp() { return app; }
module.exports = { createApp, app, prepareDemoAccounts };
